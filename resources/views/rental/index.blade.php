...
<div class="">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h1>Rental</h1>
            <a href="{{ route('rental.create') }}" class="btn btn-primary">Tambah Rental</a>
        </div>
        @if (session('success'))
    ...
    @endif
        <div class="card-body">
            <table class="table table-bordered" id="hotel-table">
                ...
                <tbody>
                    @foreach ($rentals as $rental => $r)
                    <tr>
                        <td>{{ $rental + 1 }}</td>
                        <td>{{ $r->mobil->type_kendaraan }}</td>
                        <td>{{ $r->mobil->nomor_polisi }}</td>
                        <td>{{ $r->tanggal_awal_sewa->translatedFormat('d F Y') }}</td>
                        <td>{{ $r->tanggal_akhir_sewa->translatedFormat('d F Y') }}</td>
                        <td>
                            <div class="dropdown">
                                <button class="btn btn-link text-dark p-0" type="button" id="aksiDropdown{{ $r->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="bx bx-dots-vertical-rounded" style="font-size: 1.5rem;"></i>
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="aksiDropdown{{ $r->id }}">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('rental.edit', $r->id) }}">
                                            <i class="bx bx-edit me-2"></i> Edit
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="{{ route('rental.show', $r->id) }}">
                                            <i class="bx bx-show me-2"></i> Show
                                        </a>
                                    </li>
                                    <li>
                                        <button type="button" class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deleteModalRental{{ $r->id }}">
                            ...
                            <!-- Delete Modal -->
                            <div class="modal fade modal-confirm" id="deleteModalRental{{ $r->id }}" tabindex="-1" aria-labelledby="deleteModalLabelRental{{ $r->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content text-center p-4">
                                        ...
                                        <div class="modal-body p-0">
                                            <h4 class="fw-bold mb-2" style="font-size:1.25rem;">Apakah Anda yakin ingin menghapus data rental ini?</h4>
                                            <div class="mb-4 text-muted" style="font-size:1rem;">"{{ $r->mobil->nomor_polisi }}"</div>
                                        </div>
                                        <div class="d-flex gap-2 justify-content-center mt-2">
                                            <button type="button" class="btn btn-outline-secondary btn-lg px-4" data-bs-dismiss="modal">Batal</button>
                                            <form action="{{ route('rental.destroy', $r->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
