    ...
    @endif
        <div class="card-body">
            <table class="table table-bordered" id="hotel-table">
                ...
                <tbody>
                    @foreach ($nilais as $nilai => $n)
                    <tr>
                        <td>{{ $nilai + 1 }}</td>
                        <td>{{ $n->nama_mahasiswa }}</td>
                        <td>{{ $n->nim }}</td>
                        <td>{{ $n->nilai_rata_rata }}</td>
                        <td>
                            @if ($n->nilai_rata_rata < 50)
                                <span class="badge bg-danger">Tidak Lulus</span>
                            @elseif($n->nilai_rata_rata <= 70)
                        ...
                        <td>
                            <div class="dropdown">
                                <button class="btn btn-link text-dark p-0" type="button" id="aksiDropdown{{ $n->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="bx bx-dots-vertical-rounded" style="font-size: 1.5rem;"></i>
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="aksiDropdown{{ $n->id }}">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('nilai.edit', $n->id) }}">
                                            <i class="bx bx-edit me-2"></i> Edit
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="{{ route('nilai.show', $n->id) }}">
                                            <i class="bx bx-show me-2"></i> Show
                                        </a>
                                    </li>
                                    <li>
                                        <button type="button" class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deleteModalNilai{{ $n->id }}">
                            ...
                            <!-- Delete Modal -->
                            <div class="modal fade modal-confirm" id="deleteModalNilai{{ $n->id }}" tabindex="-1" aria-labelledby="deleteModalLabelNilai{{ $n->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content text-center p-4">
                                        ...
                                        <div class="modal-body p-0">
                                            <h4 class="fw-bold mb-2" style="font-size:1.25rem;">Apakah Anda yakin ingin menghapus data nilai ini?</h4>
                                            <div class="mb-4 text-muted" style="font-size:1rem;">"{{ $n->nama_mahasiswa }}"</div>
                                        </div>
