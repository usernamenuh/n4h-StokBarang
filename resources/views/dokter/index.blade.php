                ...
                <tbody>
                    @foreach ($dokters as $dokter => $key)
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($key->jam_akhir_praktik)->translatedFormat('d F Y') }}</td>
                            <td>
                                @if ($key->status == 'booking')
                                    <span class="badge bg-label-success">Booking</span>
                                @else
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="aksiDropdownDokter{{ $key->id }}">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('dokter.edit', $key->id) }}">
                                                <i class="bx bx-edit me-2"></i> Edit
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('dokter.show', $key->id) }}">
                                                <i class="bx bx-show me-2"></i> Show
                                            </a>
                                        </li>
                                        <li>
                                            <button type="button" class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deleteModalDokter{{ $key->id }}">
                        ...
                        <!-- Delete Modal -->
                        <div class="modal fade modal-confirm" id="deleteModalDokter{{ $key->id }}" tabindex="-1" aria-labelledby="deleteModalLabelDokter{{ $key->id }}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content text-center p-4">
                                    ...
                                    <div class="modal-body p-0">
                                        <h4 class="fw-bold mb-2" style="font-size:1.25rem;">Apakah Anda yakin ingin menghapus dokter ini?</h4>
                                        <div class="mb-4 text-muted" style="font-size:1rem;">"{{ $key->nama_dokter }}"</div>
                                    </div>
                                    <div class="d-flex gap-2 justify-content-center mt-2">
                                        <button type="button" class="btn btn-outline-secondary btn-lg px-4" data-bs-dismiss="modal">Batal</button>
                                        <form action="{{ route('dokter.destroy', $key->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
